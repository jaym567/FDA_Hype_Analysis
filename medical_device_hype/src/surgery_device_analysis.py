...
class SurgeryDeviceAnalyzer:
    def __init__(self, fda_data_file=None, openalex_email=None, verbose=True, config_file=None):
        """
        Initialize the Surgery Device Analyzer
        
        Args:
            fda_data_file (str): Path to the FDA PMA data file (defaults to data/fda_pma_comprehensive.json)
            ...
            config_file (str): Path to filtering configuration file (defaults to data/surgical_device_filter_config.json)
        """
        from pathlib import Path
        if fda_data_file is None:
            data_dir = Path(__file__).parent.parent / 'data'
            self.fda_data_file = data_dir / "fda_pma_comprehensive.json"
        else:
            self.fda_data_file = Path(fda_data_file)
        ...
        self.config = self._load_filtering_config(config_file)
        self.surgical_specialties = self.config.get('surgical_specialties', {})
        self.specific_surgical_terms = self.config.get('specific_surgical_terms', [])
        self.exclusion_patterns = self.config.get('exclusion_patterns', [])
        self.surgical_robot_brands = self.config.get('surgical_robot_brands', [])
        self.exclusion_exceptions = self.config.get('exclusion_exceptions', [])
        self.filtering_settings = self.config.get('filtering_settings', {})
    ...
    def _load_filtering_config(self, config_file=None):
        ...
        if config_file is None:
            data_dir = Path(__file__).parent.parent / 'data'
            config_file = data_dir / 'surgical_device_filter_config.json'
        else:
        ...
        try:
            if config_file.exists():
                with open(config_file, 'r') as f:
                    config = json.load(f)
                if self.verbose:
                    console.print(f"✅ [green]Loaded filtering configuration from: {config_file}[/green]")
                return config
            else:
                if self.verbose:
                    console.print(f"⚠️ [yellow]Config file not found: {config_file}[/yellow]")
                    console.print("📋 [cyan]Using default filtering configuration[/cyan]")
    ...
    def _get_default_config(self):
        ...
        return {
            ...
            'surgical_robot_brands': [
                'surgical robot', 'robotic surgery', 'da vinci', 'mako', 'rosa'
            ],
    ...
    def filter_surgery_devices(self):
        ...
        # Add specific surgical terms from config
        surgery_keywords.extend(self.specific_surgical_terms)
        
        # Get exclusion patterns from config
        exclusion_patterns = self.exclusion_patterns
        exclusion_exceptions = self.exclusion_exceptions
            ...
            for exclusion in exclusion_patterns:
                if exclusion in all_text:
                    ...
                    if 'surgical' in all_text or 'surgery' in all_text:
                        # Allow if it's clearly surgical and matches an exception pattern
                        if 'surgical' in all_text and exclusion in exclusion_exceptions:
        ...
        if len(self.surgery_devices) > 0:
            ...
            def has_specific_surgical_term(row):
                ...
                # Also allow if it has surgical robot terms from config
                if any(term in all_text for term in self.surgical_robot_brands):
                    return True
                
                return False
            
            # Further filter to require specific surgical terms (if enabled in config)
            if self.filtering_settings.get('require_specific_surgical_term', True):
